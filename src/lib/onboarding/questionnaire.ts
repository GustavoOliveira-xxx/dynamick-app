import type { DimensionKey } from '@/lib/domain';

/**
 * Questionário de onboarding v1.
 * Implementa literalmente a §3 do ONBOARDING_SPEC.md — os efeitos nas dimensões
 * são os mesmos declarados lá, e os testes comparam os dois.
 */
...
export type QuestionOption = {
  ...
  /** Marca a resposta "não sei ainda": nunca penaliza, sugere diagnóstico leve. */
  triggersDiagnostic?: boolean;
  /** Apoio prático oferecido quando a opção é marcada (etapa F). */
  support?: string;
};
...
export type Question = {
  ...
  prompt: string;
  help?: string;
  type: 'single' | 'multiple' | 'scale-per-area' | 'number';
  required?: boolean;
...
export const ONBOARDING_STEPS: OnboardingStep[] = [
    ...
    questions: [
        ...
        options: [
          {
            value: 'nunca',
            label: 'Ainda não',
            effect: { needsDirection: 15, perceivedBase: -10 },
          },
          {
            value: 'por_conta',
            label: 'Estudei por conta própria, sem plano',
            effect: { autonomy: 5, needsDirection: 5 },
          },
          {
            value: 'cronograma',
            label: 'Sigo ou já segui um cronograma ou cursinho',
            effect: { autonomy: 10, perceivedBase: 10 },
          },
          {
            value: 'nao_sei',
            label: 'Não sei ainda',
      ...
      },
      {
        ...
        options: [
          ...
          {
            value: 'prova_proxima',
            label: 'Uma prova em menos de 6 meses',
            effect: { practicePreference: 15 },
          },
          { value: 'sem_data', label: 'Ainda sem data definida' },
          {
            value: 'explorando',
            label: 'Só estou explorando',
            effect: { needsDirection: 10 },
          },
        ],
      },
    ],
    ...
    questions: [
      {
        ...
        options: [
          {
            value: 'base',
            label: 'Construir base com calma',
            effect: { perceivedBase: -5, needsDirection: 5 },
          },
          {
            value: 'desempenho',
            label: 'Melhorar meu desempenho',
            effect: { practicePreference: 10 },
          },
          {
            value: 'prova_proxima',
            label: 'Me preparar para uma prova próxima',
            effect: { practicePreference: 15 },
          },
          {
            value: 'rotina',
            label: 'Organizar uma rotina de estudos',
            effect: { consistency: -10, needsDirection: 10 },
          },
          {
            value: 'curso',
            label: 'Testar possibilidades de curso',
            effect: { needsDirection: 5 },
          },
        ],
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        required: true,
        allowCustom: true,
        customLabel: 'Outro tempo (em minutos)',
        options: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: 'pouco', label: 'Varia um pouco' },
          {
            value: 'muito',
            label: 'Muda bastante',
            effect: { consistency: -20, needsShortSessions: 10 },
          },
        ],
      },
    ],
    ...
    questions: [
      {
        ...
        options: [
          ...
          {
            value: 'sequencia_questoes',
            label: 'Uma sequência de questões',
            effect: { practicePreference: 15 },
          },
          { value: 'revisao_erros', label: 'Rever meus erros', effect: { practicePreference: 5 } },
          {
            value: 'cronometrada',
            label: 'Uma sessão cronometrada',
            effect: { practicePreference: 10 },
          },
  ...
  },
  {
    ...
    questions: [
      {
        id: 'selfAssessment',
        prompt: 'Como você se sente em cada área hoje?',
        type: 'scale-per-area',
      },
    ],
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        type: 'single',
        options: [
          { value: 'auto', label: 'Seguir a configuração do meu sistema' },
          { value: 'always', label: 'Sempre reduzir animações' },
        ],
      },
      {
        id: 'highContrast',
      ...
      {
        id: 'visualIntensity',
        prompt: 'Fundo animado e efeitos visuais',
        type: 'single',
        options: [
...
export const OPTIONAL_STEPS = ONBOARDING_STEPS.filter((step) => !step.required).map((s) => s.slug);
...
export function nextStepSlug(slug: string): string | null {
...
export function firstIncompleteStep(completed: string[], skipped: string[]): string | null {
  const done = new Set([...completed, ...skipped]);
  for (const step of ONBOARDING_STEPS) {
    if (!done.has(step.slug)) return step.slug;
  }
  return null;
}

export function findQuestion(questionId: string): { step: OnboardingStep; question: Question } | null {
  for (const step of ONBOARDING_STEPS) {
...
export function findOption(questionId: string, value: string): QuestionOption | null {
  const found = findQuestion(questionId);
  if (!found?.question.options) return null;
