import type { MasteryState } from '@/lib/domain';

/**
 * Domínio de tópico — §5.2 e regra 18 da §7.
 *
...
export type MasteryInput = {
  attemptCount: number;
  correctCount: number;
  /** Questões distintas respondidas (não tentativas repetidas na mesma questão). */
  distinctQuestions: number;
  /** Sessões distintas em que o tópico apareceu. */
  ...
  /** O conteúdo do tópico foi aberto pelo menos uma vez. */
...
export function accuracy(input: MasteryInput): number {
  if (input.attemptCount <= 0) return 0;
  return input.correctCount / input.attemptCount;
}

export function computeMasteryState(input: MasteryInput): MasteryState {
...
/** Estimativa de domínio 0..100 — usada só como indicador visual, nunca como nota. */
