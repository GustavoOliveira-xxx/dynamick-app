...
import { computeMasteryScore, computeMasteryState, type MasteryInput } from './mastery';
...
describe('computeMasteryState', () => {
  ...
  it('NUNCA declara domínio com base em uma única questão (§5.2)', () => {
    ...
    expect(state).not.toBe('consolidated');
  });
  ...
  it('consolidado exige questões diferentes E mais de uma sessão (§7 regra 18)', () => {
    const mesmaSessao = computeMasteryState(
      input({ attemptCount: 5, correctCount: 5, distinctQuestions: 5, distinctSessions: 1 }),
    );
    expect(mesmaSessao).toBe('practicing');

    const duasSessoes = computeMasteryState(
      input({ attemptCount: 5, correctCount: 5, distinctQuestions: 5, distinctSessions: 2 }),
    );
    expect(duasSessoes).toBe('consolidated');
  });
    ...
    expect(
      computeMasteryState(
        input({
          ...
          distinctQuestions: 6,
          distinctSessions: 2,
          lowConfidenceAttempts: 3,
        }),
      ),
    ).toBe('needs_review');
  });

  it('esquecimento provável devolve um tópico antigo para revisão', () => {
    expect(
      computeMasteryState(
        input({
          attemptCount: 10,
...
describe('computeMasteryScore', () => {
  it('nunca sai da faixa 0..100', () => {
    ...
    for (const item of cases) {
      const score = computeMasteryScore(item);
  ...
  it('mais evidência com o mesmo acerto vale mais', () => {
    const pouca = computeMasteryScore(
      input({ attemptCount: 2, correctCount: 2, distinctQuestions: 2, distinctSessions: 1 }),
    );
    const muita = computeMasteryScore(
      input({ attemptCount: 8, correctCount: 8, distinctQuestions: 8, distinctSessions: 3 }),
    );
  ...
  it('tempo sem praticar reduz a estimativa', () => {
    ...
    expect(computeMasteryScore({ ...base, daysSinceLastPractice: 60 })).toBeLessThan(
      computeMasteryScore({ ...base, daysSinceLastPractice: 3 }),
    );
  });
});
